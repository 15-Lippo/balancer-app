...
const checkCalledWith = (calledWithStack, actualArgs) => {
  ...
  return calledWithInstance ? calledWithInstance.calledWithFn(...actualArgs) : void 0;
};
const calledWithFn = () => {
  const fn = vi.fn();
  let calledWithStack = [];
  fn.calledWith = (...args) => {
    const calledWithFn2 = vi.fn();
    if (!fn.getMockImplementation() || fn.getMockImplementation()?.name === "implementation") {
      fn.mockImplementation((...args2) => checkCalledWith(calledWithStack, args2));
      calledWithStack = [];
    }
